...
local T1UIColor = {
    ...
    ["Logo Image"] = "rbxassetid://87038715291154",
...
btnHide.Draggable = true

local btnHideFrame = Instance.new('Frame', btnHide)
btnHideFrame.AnchorPoint = Vector2.new(0.5, 0.5)
btnHideFrame.Size = UDim2.new(1, 0, 1, 0)
btnHideFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
btnHideFrame.Name = "toggleFrame"
btnHideFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
btnHideFrame.BackgroundTransparency = getgenv().UIToggled and 0 or 0.25
...
-- UIStroke cho viền
local btnStroke = Instance.new("UIStroke", btnHideFrame)
btnStroke.Color = Color3.fromRGB(60, 60, 60)
btnStroke.Thickness = 1.5
btnStroke.Transparency = 0.7

-- HÀM TOGGLE MỚI - KHÔNG HIỆU ỨNG TWEEN --
Library.ToggleUI = function()
    getgenv().UIToggled = not getgenv().UIToggled
    
    -- THAY ĐỔI TRỰC TIẾP - KHÔNG TWEEN
    if getgenv().UIToggled then
        -- Khi bật UI
        imgHide.Size = UDim2.new(0, 40, 0, 40)
        btnHideFrame.BackgroundTransparency = 0
        btnHideFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    else
        -- Khi tắt UI
        imgHide.Size = UDim2.new(0, 30, 0, 30)
        btnHideFrame.BackgroundTransparency = 0.25
        btnHideFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    end
    
    -- GIỮ NGUYÊN TÍNH NĂNG BẬT/TẮT GUI
    if game.CoreGui:FindFirstChild("Nousigi Hub GUI") then
...
btnHide.MouseEnter:Connect(function()
    btnHideFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
end)

btnHide.MouseLeave:Connect(function()
    if getgenv().UIToggled then
        btnHideFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    else
        btnHideFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
...
-- KÉO THẢ VẪN HOẠT ĐỘNG (giữ nguyên từ code cũ)
if true then
    -- GIỮ NGUYÊN TOÀN BỘ CODE KÉO THẢ TỪ SOURCE CỦA BẠN
    -- (phần từ "local button = btnHide" đến hết)
end
...
local libCreateNoti = function(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain) .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Title
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain) .. "</font> " .. tostring(getgenv().MainDesc or "")
    ...
    -- ThĂŞm biáşżn Äáť lĆ°u thĂ´ng tin section
    local sectionInfo = {}
    
    -- TáşĄo hĂ m GlobalSearch náşżu chĆ°a táťn táşĄi
    if not GlobalSearch then
        GlobalSearch = function(searchText)
            searchText = string.lower(searchText)
            
            if searchText == "" then
                -- Hiáťn tháť táşĽt cáşŁ nhĆ° cĹŠ
            ...
            for _, control in pairs(getgenv().AllControls) do
                ...
                -- ÄĂĄnh dáşĽu section cĂł pháş§n táť­ kháťp
            ...
            for section, elements in pairs(elementsInSection) do
                ...
                -- Kiáťm tra section cĂł kháťp khĂ´ng
                ...
                for _, elementInfo in ipairs(elements) do
                    ...
                    if elementInfo.elementFound then
                        -- Pháş§n táť­ kháťp: hiáťn tháť pháş§n táť­
                        control.Element.Visible = true
                        
                        -- Náşżu section cĹŠng kháťp hoáşˇc cĂł pháş§n táť­ kháťp: hiáťn section
                    ...
                    elseif elementInfo.sectionFound and not hasElementMatch then
                        -- Section kháťp nhĆ°ng khĂ´ng cĂł pháş§n táť­ kháťp: cháť hiáťn section
                        control.Section.Visible = true
                        control.Element.Visible = false
            ...
            -- Hiáťn tháť cĂĄc tab cĂł káşżt quáşŁ
            ...
            -- Náşżu khĂ´ng tĂŹm tháşĽy gĂŹ cáşŁ, hiáťn tháť thĂ´ng bĂĄo
            if not next(foundTabs) then
                -- CĂł tháť thĂŞm thĂ´ng bĂĄo "KhĂ´ng tĂŹm tháşĽy káşżt quáşŁ" áť ÄĂ˘y náşżu muáťn
            end
        end
    end
    
    -- Káşżt náťi sáťą kiáťn search (giáťŻ nguyĂŞn)
	...
	function Main_Function:AddTab(PageName)
        ...
        function sectionFunction:AddButton(Setting, Callback)
            ...
            TextColor_1.Parent = RowBG_1
             ...
             ClickArea_1.ClipsDescendants = true  -- THĂM DĂNG NĂY: NgÄn ripple trĂ n ra
...
UIGradient_1.Color = ColorSequence.new{
    ...
    ColorSequenceKeypoint.new(0.4, Color3.fromRGB(240, 240, 240)), -- Xám cực nhạt
    ColorSequenceKeypoint.new(0.6, Color3.fromRGB(230, 230, 230)), -- Xám nhạt
    ColorSequenceKeypoint.new(1, Color3.fromRGB(220, 220, 220))    -- Xám nhạt đậm hơn chút
}
             ...
             -- UIScale máşˇc Äáťnh
             UIScale_1.Scale = 1
             
             -- HOVER (cháť phĂłng to)
             ...
             Button_1.MouseLeave:Connect(function()
                ...
                Button_1.MouseButton1Down:Connect(function()
                    ...
                    -- LáşĽy kĂ­ch thĆ°áťc tháťąc táşż cáť§a ClickArea
                    local w = ClickArea_1.AbsoluteSize.X
                    local h = ClickArea_1.AbsoluteSize.Y
                    
                    -- TáşĄo ripple váťi hĂŹnh dáşĄng bo gĂłc giáťng button (cháťŻ nháş­t bo gĂłc)
                    ...
                    -- TáşĄo UICorner cho ripple váťi bo gĂłc y háťt button
                    local rippleCorner = Instance.new("UICorner")
                    rippleCorner.CornerRadius = UICorner_3.CornerRadius -- LáşĽy gĂłc bo táťŤ button
                    rippleCorner.Parent = ripple
                    
                    -- Animation ripple máť ráťng táťŤ tĂ˘m ra Äáş§y Äáť§ button
            ...
            function sectionFunction:AddDropdownSection(Setting)
                ...
                -- TáşĄo internal section Äáť cháťŠa cĂĄc control
                ...
                -- TáşĄo dropdown section functions (CHáť CĂ SLIDER)
                local dropdownSectionFunction = {}
                
                -- HĂM Táş O SLIDER (RáťNG HĆ N, SĂT VIáťN)
                function dropdownSectionFunction:AddSlider(Setting)
                    ...
                    SliderFrame.Size = UDim2.new(1, 0, 0, 50)  -- Chiáşżm toĂ n báť chiáťu ráťng
                    ...
                    SliderBG.Size = UDim2.new(1, -5, 1, 0)  -- Chiáşżm gáş§n toĂ n báť (tráťŤ 5 pixel)
                    ...
                    SliderTitle.Size = UDim2.new(0.65, -10, 0, 25)  -- Title chiáşżm 65%
                    ...
                    SliderBar.Size = UDim2.new(0.9, 0, 0, 6)  -- Thanh slider ráťng 90%
                    ...
                    Sliderboxframe.Size = UDim2.new(0.25, 0, 0, 25)  -- Textbox chiáşżm 25%
			...
			function sectionFunction:AddDropdown(idk, Setting)
				...
				local ListNew
                local OrderedList = {} -- ThĂŞm biáşżn lĆ°u tháťŠ táťą
                if Selected then
                    ListNew = {}
                    for _, value in ipairs(List) do
                        -- Kiáťm tra náşżu value trĂšng váťi Default thĂŹ set true
                        ListNew[value] = (value == Default)
                        table.insert(OrderedList, value) -- LĆ°u tháťŠ táťą
                    end
                else
                    ListNew = List
                end
				...
				local controlData = {
                    ...
                    TabName = Page_Name,
                    TabButton = PageName,
                    SetValue = dropdownFunction.SetValue,  -- THĂM DĂNG NĂY
                    GetValue = dropdownFunction.GetValue   -- THĂM DĂNG NĂY
                }
                table.insert(getgenv().AllControls, controlData)
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    local Mode = Setting.Mode or "Toggle" -- Hold hoáşˇc Toggle
